...
def process_images_2(image_list, models, user_choice_data, input_prompt, input_prompt2, image_folder, csv_rows, user_choice, description):
    """
    Main loop to process images with models with 2 input prompts in a row
    Returns classification data and error data
    """
    classification_data = {model['name']: [] for model in models}
    error_data = {model['name']: 0 for model in models} # error tracking
    for model in models:
        ...
        for i, image_name in enumerate(image_list):
            model_response, time_taken = single_call(model_name, input_prompt, os.path.join(image_folder, image_name), model['temperature'])
            model_response2, time_taken2 = single_call(model_name, input_prompt2 + model_response, None, model['temperature'])

            response = model_response + "\n\n second prompt :\n" + model_response2

            classif = classify_response(model_response2.lower())
            classification_data[model_name].append((image_name, classif)) # for plot
            ...
            print_response(model_name, image_name, classif, response, time_taken+time_taken2, user_choice, i, description, len(image_list))
            ...
            base_idx = 3 + (model_idx * 3)
            csv_rows[image_name][base_idx] = classif
            csv_rows[image_name][base_idx + 1] = response
            csv_rows[image_name][base_idx + 2] = round(time_taken, 3)
